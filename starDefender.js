...
window.addEventListener('keydown', (e) => {
  ...
  if (gameState.screen === 'LOGIN') {
    if (e.key === 'Backspace') {
      if (gameState.passFocused) {
        gameState.passInput = gameState.passInput.slice(0, -1);
      } else {
...
canvas.addEventListener('mousedown', (e) => {
  ...
  if (gameState.screen === 'LOGIN') {
    if (clickInside(clickX, clickY, loginButton)) {
      loginUser(gameState.userInput, gameState.passInput);
    } else if (clickInside(clickX, clickY, createAccountButton)) {
      registerUser(gameState.userInput, gameState.passInput);
    }
    if (clickInside(clickX, clickY, userField)) {
      gameState.passFocused = false;
    } else if (clickInside(clickX, clickY, passField)) {
      gameState.passFocused = true;
    }
    return;
  }
  if (gameState.screen === 'MENU') {
    if (clickInside(clickX, clickY, newGameButton)) {
      startNewGame();
    }
  }
});
...
function resetGameState() {
  gameState.player.x = 400 - 25;
  gameState.player.y = 600 - 80;
...
function enemyShoot(enemy) {
  ...
  const now = Date.now();
  const cooldown = enemy.isBoss ? 1500 : 3000;
  if (!enemy.lastShotTime || now - enemy.lastShotTime > cooldown) {
    if (enemy.isBoss) {
...
function maybeSpawnPowerUp(x, y) {
  if (Math.random() < 0.1) {
    const t = Math.random() < 0.5 ? 'fasterShots' : 'extraHealth';
...
function updateGameLogic() {
  ...
  if (gameState.wave === 10 && !gameState.waveBossSpawned && Date.now() >= gameState.bossSpawnTime) {
    spawnBoss();
  }
  if (gameState.keys['ArrowLeft'] && gameState.player.x > 0) {
    gameState.player.x -= gameState.player.speed;
  }
  if (gameState.keys['ArrowRight'] && gameState.player.x + gameState.player.width < canvas.width) {
    gameState.player.x += gameState.player.speed;
  }
  if (gameState.keys['ArrowUp'] && gameState.player.y > 0) {
    gameState.player.y -= gameState.player.speed;
  }
  if (gameState.keys['ArrowDown'] && gameState.player.y + gameState.player.height < canvas.height) {
    gameState.player.y += gameState.player.speed;
  }
  if (gameState.keys[' ']) {
    shootBullet();
